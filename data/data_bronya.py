...
# 创建布洛妮娅对象
bronya = build_character_data("布洛妮娅")


# 布洛妮娅buff
# 战技buff
def skill_buff(buff: Buff, msg: str):
    __list = [0.33, 0.363, 0.396, 0.429, 0.462, 0.495,
              0.5363, 0.5775, 0.6188, 0.66, 0.693, 0.726, 0.759, 0.792, 0.825]
    lv = buff.source.skill_lv
    if buff.source.eidolons >= 5:
        lv += 2
    leave_turn = 1
    if buff.source.eidolons >= 6:
        leave_turn = 2
    # 添加buff
    if msg == "add":
...
bronya_skill_buff = Buff(bronya, skill_buff)
...
# 布洛妮娅普通攻击
def bronya_basis_atk(entity_msg: Entity):
    # 创建击杀数
    kill_num = 0
    # 产出1点战技点
    ...
    if kill_num > 0 and entity_msg.source.exmove is False:
        character_exmove(seele, global_msg)
    return 0  # 返回0,行动结束
...
def seele_ultimate(entity_msg: Entity):
    ...
    if kill_num > 0:
        character_move(seele, global_msg)
    return 0
